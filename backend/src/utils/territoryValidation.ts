...
const normalize = (val?: string) => (val || '').trim().toLowerCase().replace(' district', '').replace(' division', '');

/**
 * Validate that the requested vendor territory belongs to the agent's jurisdiction.
 */
export function validateAgentJurisdiction(
  ...
  if (role === 'division') {
    ...
    const dirEntry = PINCODE_DIRECTORY[vPincode];
 ...
 */
export function validateGeographicConsistency(vendor: {
  state?: string;
  district?: string;
  division?: string;
  pincode?: string;
  ...
  const dirEntry = PINCODE_DIRECTORY[pin];
 ...
 */
export function validateVendorFieldFormats(data: {
  phone?: string;
  email?: string;
  panNumber?: string;
